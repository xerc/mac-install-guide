...
Prior to Homebrew version 2.6.0, released in December 2020, Homebrew contained a `brew switch` command that allowed switching between package versions. As of Homebrew 2.0.0, when it began removing old packages automatically after `brew upgrade` and running `brew cleanup` every thirty days to remove packages that are no longer current, `brew switch` became less useful.
